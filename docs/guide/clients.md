...
Press **Save** and the client is in the book, ready for their first policy.
...
| **Email** | | Reminders cannot be sent without this |
...
| **All cities** | Narrows to one city |
| **Any policy type** | Narrows to clients holding that kind of cover |
...
Each row shows the client's contact details, how many policies they hold and
when their next one expires — enough to decide who to call without opening
anyone.
...
filled in when adding them, and **Save** commits the change. Editing a client
never touches their policies.

## Archive a client

**Archive** takes a client out of the working list without deleting anything.
Their policies, history and members stay exactly as they are.
...
Archived clients carry a badge and reappear with **Include archived**. The same
button restores them.

## Delete a client

**Delete permanently** on the client's page removes the client and every policy
record belonging to them. It asks first, and nothing brings them back.
