...
import { AsyncPanel, Badge, Button, EmptyState, Pagination, useToast } from "./ui";
...
export function PolicyTable({
            ...
            render: (row: Policy) => (
              <span className="block min-w-0">
                <Link
                  ...
                  // The row opens the policy, so following the client has to
                  // stay the only thing this click does.
      ...
      render: (row) => (
        <span className="block">
          ...
          <Badge tone={urgencyTone(row.daysToExpiry, row.status)}>
            {row.isRenewed ? "Renewed" : relativeDays(row.daysToExpiry)}
          </Badge>
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-end gap-1">
          {/* A year with a later one behind it cannot be renewed again, and the
              status no longer says so on its own: a cancelled year that was
              renewed keeps saying cancelled. */}
          {onRenew && !row.isRenewed && (
            <Button
              size="sm"
              variant="subtle"
  ...
  return (
    <AsyncPanel query={policies} errorTitle="The policy list could not be read">
      <DataTable
        columns={columns}
        ...
        onSort={(key) =>
          onFilterChange({
            ...
            // Page 4 of one order is not page 4 of another, so a new order
            // starts at the top of the list it makes.
            page: 1,
          })
        }
        onRowClick={onEdit}
